...
public interface CustomerRejectedMapper {

    CustomerRejectedMapper MAPPER = Mappers.getMapper(CustomerRejectedMapper.class);

    @Mapping(target = "customer.customerFinancials", ignore = true)
    @Mapping(target = "customer.customerGoals", ignore = true)
    CustomerRejectedEvent toCustomerRejectedEvent(CustomerEntity customer);
